package puyuntech.com.beihai.ui.activity.shijian;
...
import puyuntech.com.beihai.ui.adapter.shijian.ShiJianAdapter;
...
public class ShiJianListFragment extends BaseFragment implements SwipeRefreshLayout.OnRefreshListener {
    ...
    private ArrayList<ShiJianMod> mylist;
    ...
    private void initView() {
        adapter = new ShiJianAdapter(mylist) {
            @Override
...
//                skipIntent(RenWuDeatilActivity.class, false);//跳转页面
    ...
    private void getNetData(final int page, int type, int pageSize, int count) {
        ...
        ArrayList<ShiJianMod> netlist = new ArrayList<>();//模拟网络数据
        ShiJianMod mod;
        for (int i = 0; i < 10; i++) {
            mod = new ShiJianMod(type);
            mod.content = "我家有只狗，谁帮我喂喂啊！";
            mod.title = i + "、我家有只狗";
